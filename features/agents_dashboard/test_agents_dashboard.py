...
from .renderer import (
    FONT_ROLE_FILES,
    SYSTEM_OVERLAY_DATE_BOX,
    SYSTEM_OVERLAY_SAFE_BOTTOM,
    SYSTEM_OVERLAY_TIME_BOX,
...
FONT_DIRECTORY = SOURCE_FONT_DIRECTORY


class RuntimePathTests(unittest.TestCase):
    def test_project_font_originals_are_complete(self) -> None:
        self.assertTrue(
            all(
                (SOURCE_FONT_DIRECTORY / name).is_file()
                for name in FONT_ROLE_FILES.values()
            )
        )

    def test_macos_runtime_paths_are_outside_repository(self) -> None:
        home = Path("/Users/tester")
        self.assertEqual(
            default_output_directory(platform="darwin", home=home),
            home / "Library/Application Support/Cuktech/AP01/agents-dashboard",
        )
        self.assertEqual(
            default_cache_directory(platform="darwin", home=home),
            home / "Library/Caches/Cuktech/AP01/agents-dashboard",
        )
        self.assertEqual(
            default_font_directory(platform="darwin", home=home),
            home / "Library/Application Support/Cuktech/AP01/fonts",
        )

    def test_other_runtime_paths_do_not_recreate_env_directory(self) -> None:
        home = Path("/home/tester")
        self.assertEqual(
            default_cache_directory(platform="linux", home=home),
            home / ".cuktech/AP01/agents-dashboard-cache",
        )
        self.assertEqual(
            default_font_directory(platform="linux", home=home),
            home / ".cuktech/AP01/fonts",
        )
...
class CollectorTests(unittest.TestCase):
    ...
    def test_api_rate_implementation_matches_authoritative_document(self) -> None:
        document = (
            PROJECT_ROOT / "knowledge" / "Codex-模型API计费表.md"
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for path in paths.values():
                with Image.open(path) as image:
                    ...
                    top = image.crop((0, 0, 320, SYSTEM_OVERLAY_SAFE_BOTTOM))
                    left, top_y, right, bottom = SYSTEM_OVERLAY_TITLE_BOX
                    self.assertIsNotNone(top.crop((left, top_y, right, bottom)).getbbox())
                    top.paste(
                        (0, 0, 0),
                        (left, top_y, right, bottom),
                    )
                    self.assertIsNone(top.getbbox())
                    self.assertIsNotNone(
                        image.crop((0, SYSTEM_OVERLAY_SAFE_BOTTOM, 320, 78)).getbbox()
                    )
                    self.assertEqual(
                        PAGE_TITLE_X,
                    ...
                    self.assertEqual(SYSTEM_OVERLAY_TITLE_BOX, (101, 14, 221, 38))
                    self.assertEqual(PAGE_TITLE_SIZE, 14)
