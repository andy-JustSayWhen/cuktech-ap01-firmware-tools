...
class ResultPackageTests(unittest.TestCase):
    ...
    def test_png_conversion_is_two_frame_device_gif(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertTrue(_gif_color_table_sizes(payload))
            self.assertLessEqual(
                max(_gif_color_table_sizes(payload)),
                DEVICE_GIF_COLORS,
            )
            self.assertEqual(hashlib.sha256(payload).digest_size, 32)

    def test_png_conversion_preserves_second_frame_on_dark_dashboard(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            source = root / "dark-dashboard.png"
            target = root / "dark-dashboard.gif"
            page = Image.new("RGB", (320, 240), (0, 0, 0))
            for index, color in enumerate(
                ((245, 245, 242), (255, 212, 0), (0, 213, 255), (103, 24, 0))
            ):
                left = 20 + index * 60
            ...
            png_to_device_gif(source, target)
    ...
    def test_png_conversion_preserves_source_colors_exactly(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            source = root / "colors.png"
            target = root / "colors.gif"
            page = Image.new("RGB", (320, 240), (0, 0, 0))
            colors = (
                (245, 245, 242),
                (255, 212, 0),
            ...
            self.assertEqual(decoded.getpixel((0, 20)), colors[0])
            self.assertEqual(decoded.getpixel((10, 20)), colors[1])
            self.assertEqual(decoded.getpixel((20, 20)), colors[2])
            self.assertEqual(decoded.getpixel((30, 20)), colors[3])
            self.assertEqual(decoded.getpixel((40, 20)), colors[4])
            self.assertEqual(decoded.getpixel((50, 20)), colors[5])

    def test_weekly_frames_cover_batches_and_single_batch_gate(self) -> None:
        cards = tuple(
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with Image.open(target) as image:
                self.assertEqual(image.n_frames, 2)
            self.assertLessEqual(
                max(_gif_color_table_sizes(target.read_bytes())),
                DEVICE_GIF_COLORS,
            )
            single_target = Path(directory) / "weekly-single.gif"
            weekly_to_device_gif(
                replace(
                    snapshot,
            ...
            with Image.open(single_target) as image:
                self.assertEqual(image.n_frames, 2)
            self.assertLessEqual(
                max(_gif_color_table_sizes(single_target.read_bytes())),
                DEVICE_GIF_COLORS,
            )

    def test_png_conversion_uses_full_gif_palette_without_dither(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            for index in range(257):
                image.putpixel((index, 0), (index, (index * 3) % 256, (index * 7) % 256))
            image.save(source)

            payload = png_to_device_gif(source, target)

            self.assertLessEqual(max(_gif_color_table_sizes(payload)), DEVICE_GIF_COLORS)
    ...
    def test_saved_result_survives_new_bridge_state(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(restarted.package_for_request(), package)
