...
class InteractionContract:
    ...
    fixed_hidden_primary_pages_enabled: bool = False
    weather_hidden_primary_page_enabled: bool = False
    calendar_skip_direction_correct: bool = False
    overview_left_preserves_gif_until_switch: bool = False
...
class TraceStep:
    ...
    intermediate_visible_pages: tuple[str, ...] = ()
    resolved: bool = True
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "intermediate_visible_pages": list(self.intermediate_visible_pages),
...
def _stock_transition(
    ...
    if contract.fixed_hidden_primary_pages_enabled:
        power_available = state.base_connected and state.power_data_available
        stock_pages = tuple(
            ([0] if power_available else [])
            + ([1] if configuration.c1_enabled else [])
            + ([2] if configuration.c2_enabled else [])
            + [3, 4, 5, 7, 6]
        )
        if state.dispatch == 0 and not power_available:
            target = stock_pages[0] if event == "left" else stock_pages[-1]
        elif state.dispatch not in stock_pages:
            raise InteractionSimulationError("当前原厂分派序号不在启用页面序列")
        else:
            offset = 1 if event == "left" else -1
            target = stock_pages[
                (stock_pages.index(state.dispatch) + offset) % len(stock_pages)
            ]
        if target == 4:
            if contract.calendar_skip_direction_correct:
                offset = 1 if event == "left" else -1
                target = stock_pages[
                    (stock_pages.index(target) + offset) % len(stock_pages)
                ]
            else:
                target = state.dispatch
        if target == 5 and contract.weather_hidden_primary_page_enabled:
            offset = 1 if event == "left" else -1
            target = stock_pages[
                (stock_pages.index(target) + offset) % len(stock_pages)
            ]
...
def simulate_event(
    ...
    *,
    ...
    intermediate_visible_pages: tuple[str, ...] = ()
    if (
        contract.fixed_hidden_primary_pages_enabled
        and state.agents_state == 1
        and event == "left"
        and route.action == "switch-stock"
        and not contract.overview_left_preserves_gif_until_switch
    ):
        intermediate_visible_pages = ("pet",)
    ...
    return TraceStep(
        ...
        intermediate_visible_pages=intermediate_visible_pages,
    )
...
def run_interaction_simulation(
    ...
    *,
    ...
    if (
        contract.fixed_hidden_primary_pages_enabled
        and not contract.calendar_skip_direction_correct
    ):
        failures.append(
            SimulationFailure(
                code="CALENDAR_SKIP_DIRECTION_REVERSED",
                scenario="binary-contract",
                message="日历双向跳过目标会回到当前页",
            )
        )
    if (
        contract.fixed_hidden_primary_pages_enabled
        ...
        failures.append(
            SimulationFailure(
                code="PET_INTERMEDIATE_FRAME_REACHABLE",
                scenario="binary-contract",
                message="看板切到设置前会先恢复萌宠画面",
            )
        )

    configurations = (
        tuple(
            PageConfiguration(c1, c2, False, False, True)
            for c1, c2 in product((False, True), repeat=2)
        )
        if contract.fixed_hidden_primary_pages_enabled
        else
        tuple(
            PageConfiguration(c1, c2, True, True, True)
            for c1, c2 in product((False, True), repeat=2)
        )
        if contract.fixed_shared_pages_enabled
        else tuple(
            PageConfiguration(c1, c2, calendar, pet, agents)
            for c1, c2, calendar, pet, agents in product((False, True), repeat=5)
        )
    )
    for configuration in configurations:
        ...
        for direction in ROTATION_EVENTS:
            ...
            required_stock = {
                ...
                if (
                    (index != 7 or configuration.pet_enabled)
                    and not (
                        index == 5
                        and contract.weather_hidden_primary_page_enabled
                    )
                )
            }
    ...
    if contract.power_confirm_guard_enabled:
        detached_configuration = (
            PageConfiguration(calendar_enabled=False, pet_enabled=False)
            if contract.fixed_hidden_primary_pages_enabled
            else PageConfiguration()
        )
        for direction in ROTATION_EVENTS:
    ...
    detail_scenarios = {
        ...
        "overview-left-exit": (InteractionState(7, 1), ("left",)),
    ...
    exhaustive_starts = tuple(
        ...
        for dispatch in exhaustive_dispatches
...
def contract_from_manifest(
    ...
    *,
    ...
    if manifest_type not in (
        ...
        "agents-live-data-fixed-primary-pages-firmware",
        "agents-live-data-fixed-primary-pages-v2-firmware",
        "agents-live-data-weather-coexistence-firmware",
        "agents-live-data-weather-round-robin-firmware",
        "agents-live-data-round-diagnostic-firmware",
        "agents-download-diagnostic-firmware",
        "agents-result-diagnostic-firmware",
        "agents-publish-diagnostic-firmware",
        "agents-adoption-diagnostic-firmware",
        "agents-stable-adoption-firmware",
        ...
        "agents-weather-hidden-dashboard-firmware",
    ...
    raw_transport = document.get("transport")
    ...
    location_independent = manifest_type in (
        "agents-live-data-location-independent-firmware",
        "agents-live-data-validated-package-firmware",
        "agents-live-data-endpoint-failover-firmware",
        "agents-live-data-fixed-primary-pages-firmware",
    ...
    fixed_hidden_primary_pages = manifest_type in (
        "agents-live-data-fixed-primary-pages-firmware",
        "agents-live-data-fixed-primary-pages-v2-firmware",
        "agents-live-data-weather-coexistence-firmware",
        "agents-live-data-weather-round-robin-firmware",
    ...
    return InteractionContract(
        ...
        overview_right_target_dispatch=(
            0 if stock_safe or fixed_hidden_primary_pages else None
        ),
        power_left_enters_agents=(
            stock_safe
            or fixed_hidden_primary_pages
            or any(label == "功率左旋" for label in labels)
        ),
        stock_entry_filter_enabled=bool(
        ...
        fixed_shared_pages_enabled=(stock_safe or fixed_hidden_primary_pages),
        fixed_hidden_primary_pages_enabled=fixed_hidden_primary_pages,
        weather_hidden_primary_page_enabled=bool(
            transport.get("weather_hidden_primary_page")
        ),
        calendar_skip_direction_correct=bool(
            validation.get("calendar_skip_direction_verified")
        ),
        overview_left_preserves_gif_until_switch=bool(
            validation.get("overview_left_gif_preserved_until_switch")
        ),
...
def simulate_manifest(
    ...
    *,
    ...
    if isinstance(output, Mapping):
        firmware_sha256 = output.get("sha256")
        if isinstance(firmware_sha256, str) and len(firmware_sha256) == 64:
