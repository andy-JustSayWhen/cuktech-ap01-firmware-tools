...
from features.primary_page_settings.single_iteration_append import (
    MENU_DISPATCH_A,
...
class PageSettingsSingleIterationAppendTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        if not STAGE.is_file() or not all(item.is_file() for item in TOOLS.values()):
            raise unittest.SkipTest("本机缺少 A 阶段输入或设备端构建工具")

    def test_simulation_uses_seven_iterations_for_eight_items(self) -> None:
        result = simulate_page_settings_single_iteration_append()
        self.assertTrue(result["passed"])
        self.assertEqual(result["iterations_checked"], 7)
        self.assertEqual(result["items_created"], 8)
        self.assertEqual(result["failures"], 0)
        self.assertFalse(result["sequence_7_exists"])
    ...
    def test_build_changes_only_payload_and_recovery_crc(self) -> None:
        with tempfile.TemporaryDirectory() as selected:
            ...
            self.assertEqual(document["payload"]["stock_return_create_calls"], 1)
    ...
    def test_writable_stage_is_rejected_before_build(self) -> None:
        with tempfile.TemporaryDirectory() as selected:
            ...
            with self.assertRaisesRegex(PageSettingsSingleIterationAppendError, "只读"):
                build_page_settings_single_iteration_append(
                    writable,
                    root / OUTPUT_NAME,
                    root / "manifest.json",
    ...
    def test_dirty_code_is_rejected_before_input_access(self) -> None:
        with self.assertRaisesRegex(PageSettingsSingleIterationAppendError, "尚未提交"):
            build_page_settings_single_iteration_append(
                Path("missing.bin"),
                Path(OUTPUT_NAME),
                Path("manifest.json"),
