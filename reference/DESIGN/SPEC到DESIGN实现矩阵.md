...
| `SPEC-DASHBOARD-029` 至 `SPEC-DASHBOARD-033` | [`优化固件 DESIGN`](AP01-1.0.2_0031-opt.bin.md) 第 6.1、7.1、10.6 至 10.19、14 节 | `features/agents_dashboard_firmware/` 的原厂萌宠三个局部分支、共享序号切页过滤与 `interaction_simulator.py` | 刷前连续事件模拟、概览进入详情、三详情循环、返回、一级左右导航、功率确认与全部原厂页面回归 | `FW-AGENTS-009` 已通过成品反汇编、4 种端口组合、14 个定向场景和 3,630 条穷举序列；真机验收待安装后完成 |
