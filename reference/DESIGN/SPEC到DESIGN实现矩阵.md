...
| `SPEC-DASHBOARD-029` 至 `SPEC-DASHBOARD-033` | [`优化固件 DESIGN`](AP01-1.0.2_0031-opt.bin.md) 第 6.1、7.1、10.6 至 10.16、14 节 | `features/agents_dashboard_firmware/` 的原厂萌宠三个局部分支 | 概览进入详情、三详情循环、返回、一级左右导航、功率确认与全部原厂页面回归 | `FW-AGENTS-007` 已安装、重启并重新上线；物理交互待用户验收 |
