...
| `SPEC-PAGE-005` 至 `SPEC-PAGE-007` | [`优化固件 DESIGN`](AP01-1.0.2_0031-opt.bin.md) 第 5、11、14 节 | `features/primary_page_settings/` 的保存与恢复代码、一级导航候选 | 即时生效、保存失败、断电、重启、默认值和关闭当前页 | 拟定；真机持久化证据阻塞 |
...
| `SPEC-DASHBOARD-029` 至 `SPEC-DASHBOARD-033` | [`优化固件 DESIGN`](AP01-1.0.2_0031-opt.bin.md) 第 6.1、7.1、10.6 至 10.13、14 节 | `features/agents_dashboard_firmware/` 的局部分支与后续观测载荷 | 概览进入详情、三详情循环、返回、一级左右导航和全部原厂页面回归 | 当前局部分支候选物理验收失败；下一方案拟定 |
