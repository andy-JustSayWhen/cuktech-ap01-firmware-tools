...
| `SPEC-DASHBOARD-029` 至 `SPEC-DASHBOARD-033` | [`优化固件 DESIGN`](AP01-1.0.2_0031-opt.bin.md) 第 6.1、7.1、10.6 至 10.18、14 节 | `features/agents_dashboard_firmware/` 的原厂萌宠三个局部分支与 `interaction_simulator.py` | 刷前连续事件模拟、概览进入详情、三详情循环、返回、一级左右导航、功率确认与全部原厂页面回归 | `FW-AGENTS-007`、`FW-AGENTS-008` 已否定；`FW-SIM-001` 已实现并正确拒绝当前候选，尚未取得可通过合同与真机验收 |
