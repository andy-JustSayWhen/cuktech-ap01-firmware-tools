...
| 后台实现 | Python 3.9 或更高版本的本机服务；首轮只使用标准库 |
| 页面实现 | 同一套本地网页结构、样式和交互资源，随发布包离线提供 |
| 刷机通道 | 复核并改造参考项目的小米云识别、文件上传、仅下载和正式安装能力 |
| 状态更新 | 页面每秒查询一次操作记录；没有直接证据的阶段不显示成功 |
| 写入保护 | 单活动操作、冻结设备与成品身份、仅下载与正式安装分别确认、未知状态只查询 |
| 凭据边界 | 账号、设备凭据、签名地址和网络地址只在后台内存或本机私有文件中使用 |
| 当前代码 | 尚未建立 `features/web_firmware_flash/` 和 `app/ap01_web.py` |
| 下一验证 | 先实现只读启动、准备检查、设备识别和固件核对，不接通任何写入动作 |
...
承接 `SPEC-FLASH-001` 至 `SPEC-FLASH-007`。
...
| 风险确认 | `risk` | 设备与成品身份冻结 | 确认仅下载；仅下载成功后另行确认正式安装 | 当次确认记录绑定操作、设备和成品 |
| 执行刷机 | `execute` | 对应动作当次确认有效 | 只查询；后台推进允许的单步动作 | 上传、设备下载、安装、暂时离线、重新上线和验收分别记录 |
...
| `POST /api/v1/operations/{id}/confirm-download` | 记录本次仅下载确认并启动允许的上传、回读和仅下载流程 | 是 |
| `POST /api/v1/operations/{id}/confirm-install` | 在仅下载成功且全部门禁重新通过后，记录正式安装确认并下发一次 | 是 |
| `GET /api/v1/operations/{id}` | 查询六类执行状态和证据摘要 | 否 |

修改类接口必须同时检查当前阶段、允许动作、单次操作编号、冻结身份、访问会话和是否已经执行。
同一确认接口重复到达时只返回原操作状态，不再次下发。

## 7. 操作记录、未知状态和恢复

承接 `SPEC-FLASH-005` 至 `SPEC-FLASH-007`。
...
| 设备下载 | 设备返回仅下载成功，且查询未出现对应错误 | 停止，只允许查询 |
