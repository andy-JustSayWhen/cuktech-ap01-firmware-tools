...
| 当前代码 | `features/web_firmware_flash/install_policy.py` 已实现自动推进合同；实际云端安装适配和 `app/ap01_web.py` 尚未建立 |
| 下一验证 | 实现只读启动、设备识别、操作记录和云端上传回读与正式安装适配 |
...
当前实现先落下 `features/web_firmware_flash/install_policy.py`：它只接收由应用入口组合得到的
本次操作、冻结身份、模拟结论、上传回读和设备状态，不直接调用固件功能模块。输出动作只包括
等待本次操作开始、运行模拟、上传回读、单次安装、只查询或停止，不存在二次确认动作。对应
6 项自动测试已经覆盖模拟通过自动安装、模拟失败停止、客观门禁变化停止和防重复下发。
