...
| 当前代码 | 尚未建立 `features/web_firmware_flash/` 的实际云端安装适配和 `app/ap01_web.py` |
| 下一验证 | 先实现并测试自动推进门禁，再实现只读启动、设备识别和云端安装适配 |
...
| 2026-08-01 | `WEB-002`：模拟通过后自动正式安装 | 用户明确要求模拟通过后直接刷机且不得二次询问；原方案的仅下载与安装双确认不再满足当前需求 | 保留单活动操作、冻结身份、完整回读、未知状态只查询和单次下发；删除二次确认状态与接口，先实现自动推进策略测试 |
