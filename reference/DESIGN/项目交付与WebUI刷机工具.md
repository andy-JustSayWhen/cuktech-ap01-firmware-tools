...
| 当前代码 | `features/web_firmware_flash/` 已实现六阶段页面、访问边界、设备与成品冻结、原子操作记录、云端上传完整回读、单次正式安装适配和双平台同源发布；`app/ap01_web.py` 已建立 |
...
| 2026-08-04 | `WEB-002`：完整本机入口与发布实现 | 六阶段页面、操作恢复、唯一设备、成品清单、完整对象回读、单次安装、双平台启动文件、发布扫描和文档核对已实现；全项目 161 项自动测试通过 | 当前不安装未满足最终门禁的候选；后续只对批准候选执行真实网页流程和双平台页面验收 |
