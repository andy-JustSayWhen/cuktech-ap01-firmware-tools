...
| 项目 | 当前状态 |
| --- | --- |
| 内容核对 | 用户已确认四张页面内容；整数、Token 单位、圆环语义、颜色方向和分组图标已固定 |
| 视觉评审 | 四张效果图是唯一视觉目标；修正版设备动图帧已通过同数据对照，待用户真机复核 |
...
- 使用纯黑全屏背景，不绘制设备外框、底部导航或整页容器；设备端页面对象不得在画面外
  叠加边框、内边距或滑块；
...
- 超大主数字和主要 Token 数字使用 Michroma Regular，匹配效果图中宽体、圆角、几何化的
  数字轮廓；中文、单位和其他文字使用 MiSans。次级数字使用 MiSans Medium，7 至 9 像素的
  正文、字段名和单位使用 MiSans Semibold，彩色状态标题与关键短状态使用 MiSans Bold；
  时间戳等低优先信息使用中等字重配合暗色。不得仅靠字号和颜色制造层级，也不得让整页小字
  共用同一字重；
...
> 必须分开放置并遵守 `SPEC-DASHBOARD-015` 至 `SPEC-DASHBOARD-018`。剩余额度和命中率等
> 高值更好的比例指示必须按数值
...
3. 今日消耗：总消耗使用超大整数和独立 Token 单位，请求数与 API 成本使用紧凑状态，输入、
...
用户已批准从这些效果图实现服务端渲染器并生成真实数据对照图；对照图复核前不转为固件
资源。
