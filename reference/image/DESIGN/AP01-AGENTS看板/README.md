...
- 字重固定为四级：超大主数字使用 Regular（常规字重），次级数字使用 Medium（中等字重），
  7 至 9 像素的正文、字段名和单位使用 Semibold（半粗字重），彩色状态标题与关键短状态
  使用 Bold（粗字重）；时间戳等低优先信息使用中等字重配合暗色。不得仅靠字号和颜色制造
  层级，也不得让整页小字共用同一字重；
