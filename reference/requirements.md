...
0041 故障取证按 [取证设计](DESIGN/0041故障取证.md) 使用 Capstone `5.0.7` 和 Unicorn `2.1.4`，仅用于电脑内诊断，不参与固件制作或设备安装。后者安装于被忽略的 `artifacts/build/diagnostic-deps/`，运行时通过模块搜索路径加载；诊断缺少这些工具时应明确失败，不得绕过检查。
